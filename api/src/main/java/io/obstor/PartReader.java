...
public class PartReader {
  ...
  private int readBuf16k(int length) throws ObstorException {
    try {
  ...
  private void readOneByte() throws ObstorException {
    if (eof) return;
  ...
  public void read(ByteBuffer buffer) throws ObstorException {
    if (buffer == null) throw new IllegalArgumentException("valid buffer must be provided");
    if (eof) throw new ObstorException("EOF reached");
    if (partNumber == partCount) throw new ObstorException("data fully read");
    ...
    while (bytesRead < size) {
      ...
      if ((eof = n < 0)) {
        ...
        throw new ObstorException("unexpected EOF");
      }
      try {
